...
namespace MitmProxyForTcp
{
    internal static class Program
    {
        private static readonly TcpListener Listener = new TcpListener(IPAddress.Any, 4520);
        ...
        private static void AcceptConnection(TcpClient client)
        {
            try
            {
                ...
                clientStream.AuthenticateAsServer(certificate, false, System.Security.Authentication.SslProtocols.None, false);
        ...
        private static void ReadFromClient(TcpClient client, Stream clientStream, Stream serverStream)
        {
            ...
            //using var stream = fileInfo.OpenWrite(); //for logging
            while (true)
            {
                int clientBytes;
                try
                ...
                // stream.Write(message, 0, clientBytes); //for logging
            }
            client.Close();
        }
    }
